...
export default async function AdminRequestsPage({
  ...
  const [studentsRes, requestsRes] = await Promise.all([
    supabase.from("users").select("id, name, email, korean_level, goals").eq("role", "student").order("name"),
  ...
  return (
    <>
      <PageHeader title="수업 요청 모아보기" subtitle="학생들이 다음 수업에서 배우고 싶은 내용입니다." />
      <RequestsBoard
        students={studentsRes.data ?? []}
        requests={requestsRes.data ?? []}
        initialFilter={student ?? "all"}
      />
    </>
  );
}
